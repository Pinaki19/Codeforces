...
void reach(int x,int y,auto& grid,auto & v,int &n,bool fl){
    if(!inside(x,y,n)|| !fl && v[x][y]){
        return;
    }
    if(fl){
        switch(grid[x][y]){
            case '>':
                return reach(x,y+1,grid,v,n,0);
                break;
            case '<' : 
                return reach(x, y - 1, grid, v, n, 0);
        }
    ...
    if (x == 1 && y == n - 1)
        return;
    reach(x,y-1,grid,v,n,1);
    reach(x, y + 1, grid, v, n,1);
    reach(x-1, y , grid, v, n,1);
    reach(x+1, y, grid, v, n,1);
}
...
void helper(){
  ...
  reach(0,0,grid,visited,n,false);
    if(visited[1][n-1]){
        YES;
    }else
        NO;
