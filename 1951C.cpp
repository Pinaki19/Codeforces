...
void helper(){
  ...
  vi arr(n);
  scan(arr);
  auto x=[](auto &p1,auto& p2){
    if(p1.F==p2.F){
        return p1.S>p2.S;
    }
    return p1.F>p2.F;
  };
  priority_queue<pair<int,int>,vector<pair<int,int>>,decltype(x)> pq(x);
  for(int i=0;i<n;i++){
    pq.push({arr[i],i});
  }
  ll cost=0;
  ...
  while(k>0){
    auto t=pq.top();
    pq.pop();
    int cost=t.F,ind=t.S;
    ans.push_back({ind,min(k,m)});
    k-=m;
  }
