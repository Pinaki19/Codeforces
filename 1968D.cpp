...
ll move(int cur,int n,auto& perm,auto& a,auto& dp){
    if(n==0) return 0LL;
    if(dp[{cur,n}]!=0) return dp[{cur,n}];
    return dp[{cur,n}]=a[cur]*1LL+max(move(cur,n-1,perm,a,dp),move(perm[cur],n-1,perm,a,dp));
}
...
void helper(){
  ...
  ans1=move(pb,k,perm,a,dp);
  ans2=move(ps,k,perm,a,dp);
  //cout<<ans1<<"  "<<ans2<<endl;
  if(ans1>ans2){
      cout << "Bodya";
  }else if(ans2>ans1){
