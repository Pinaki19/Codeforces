...
void helper(){
    ...
    auto getNext=[&p,&n](auto& s1,auto& s2,int ind){
        if(ind<n) return s1[ind];
        int rest=ind%n;
        return s2[p-1-rest];
    };
    int start=p;
    ...
    while(start<n){
        int i=start;
        int t=upto;
        for(int x=0;x<tot;x++,i+=k){
            for(int j=0;j<k-1;j++){
                if (getNext(s1, s2, i + j ) != getNext(s1, s2, i + j + 1)){
                    upto=i-1;
                    goto Here;
                }
            }
            for(int j=0;j<k;j++){
                if(getNext(s1,s2,i+j)==getNext(s1,s2,i+j+k)){
                    upto=i-1;
                    goto Here;
                }
            }
