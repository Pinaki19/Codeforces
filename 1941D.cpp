...
void ans(int i,int pl,auto& th,auto & s,int &n){
    if(i>=th.size()){
        s.insert(pl);
        return;
    }
    ...
    if(c=='0'){
        ...
        int next = (pl - dist + n) % n;
        ans(i + 1, next, th, s, n);
    }else{
        int next1 = (pl * 1LL + dist) % n;
        int next2 = (pl - dist + n)%n;
        ans(i + 1, next1, th, s, n);
        ans(i + 1, next2, th, s, n);
    }
}
...
void helper(){
  ...
  int i=0;
  while(i<m){
    cin>>throws[i].F>>throws[i].S;
    //cout << throws[i].F << throws[i].S;
    i++;
  }
  set<int> s;
  ans(0,x-1,throws,s,n);
    cout<<s.size()<<endl;
    for(int i:s)
        cout<<i+1<<" ";
