...
void helper(){
  ...
  for(int i=0;i<=n;i++){
    int sat1=fceil(i,2);
    int sat2=fceil(n-i,2);
    int zeroes=freq[i];
    int ones=freq2[n]-freq2[i];
    if(zeroes>=sat1 && ones>=sat2){
        if(fabs((float)n/2-ans)==fabs((float)n/2-i))
            ans=min(ans,i);
        else
            ans = fabs((float)n / 2 - ans) <fabs((float)n / 2 - i)? ans:i;
    }
  }
