...
bool exists(int x,int y,auto& M){
    return M.find({x - 1, y - 1})!= M.end() && M.find({x - 1, y + 1})!= M.end() && M.find({x + 1, y - 1}) != M.end() && M.find({x + 1, y + 1}) != M.end();
}
void helper(){
  vector<string> grid;
  for(int i=0;i<7;i++){
  ...
  while(count>0){
    ...
    if(grid[x-1][y-1]=='B' && exists(x-1,y-1,cMap)){
        grid[x - 1][y - 1]='A';
        int p=x-1,q=y-1;
        F[{p-1,q-1}]--;
        F[{p-1,q+1}]--;
