...
int getMin(int i,int curRun,int curBreak,auto &arr){
    if(i>=arr.size())
        return 0;
    else{
        return min(getMin(i+1,curRun+arr[i],curBreak,arr),max(curRun,arr[i]+getMin(i+1,0,curBreak,arr)));
    }
}
